...
export class Retorno {
  ...
  constructor(
    ...
    private OcorrenciaBancaria: OcorrenciaBancariaModel
  ) { }
      ...
      switch (line.charAt(0)) {
        ...
        case '1': // registro de transação
          ...
          if (registro.numeroControle.startsWith('Z')) {
            ...
            const boleto = await this.Boleto.findById(id);
            const cliente = await this.Cliente.findById(boleto.get('cliente'));

            if (boleto) {
              registro.boleto = boleto.toJSON();

              if (applyChanges === true) {
                const boletoProcessado = await this.aplicarOcorrenciaAoBoleto(registro, boleto);
                this.Boleto.update(boletoProcessado.toJSON(), { where: { _id: boletoProcessado.get('_id') } });
              }
   ...
   */
  private async aplicarOcorrenciaAoBoleto(registroTransacao: ITransactionRetorno, boleto: BoletoInstance): Promise<BoletoInstance> {
    ...
    const ocorrencias = await this.OcorrenciaBancaria.findAll({
      where: { boleto: boleto.get('_id') }
    });

    ocorrencias.push(await this.OcorrenciaBancaria.create(
      {
        dataHora,
        descricaoOcorrencia,
        motivoOcorrencia,
